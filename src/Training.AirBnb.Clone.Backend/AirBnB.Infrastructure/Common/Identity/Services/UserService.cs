...
public class UserService(IUserRepository userRepository, UserValidator userValidator) : IUserService
{
    ...
    {
        var validationResult = userValidator.Validate(user, options => options.IncludeRuleSets(EntityEvent.OnCreate.ToString()));
    ...
    public async ValueTask<User> GetSystemUserAsync(bool asNoTracking, CancellationToken cancellationToken)
    {
        return await userRepository.Get(asNoTracking: asNoTracking)
            .Include(user => user.Roles)
